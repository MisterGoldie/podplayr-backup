 ...
 * Bare CIDs / Alchemy hashes / Arweave tx ids are left alone.
 */
...
/** Safari/WebKit often needs a fragment type hint for extensionless QuickTime. */
export const withBrowserVideoHint = (url: string): string => {
  if (!url || /#\.[a-z0-9]{2,4}$/i.test(url) || VIDEO_FILE_EXT_RE.test(url)) return url;
  if (!urlLooksLikeExtensionlessVideo(url)) return url;
